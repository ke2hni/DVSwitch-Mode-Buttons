...
OLD_MARKER='// DVSwitch-Mode-Buttons: standalone DMR Master display v1'
NEW_MARKER='// DVSwitch-Mode-Buttons: standalone DMR Master display v2'
...
if text.count(new_marker) == 1:
    print('ALREADY INSTALLED: standalone DMR Master display v2 is present.')
    raise SystemExit(0)
if text.count(old_marker) != 1:
    raise SystemExit('ERROR: expected exactly one standalone DMR Master display v1')
...
new_display = '''function dvsButtonsDmrSavedNetwork() {
...
if text.count(old_heading) != 1 or text.count(old_display) != 1:
    raise SystemExit('ERROR: expected exactly one v1 heading and display function')
updated = text.replace(old_marker, new_marker, 1)
updated = updated.replace(old_heading, new_heading, 1).replace(old_display, new_display, 1)
...
print(f'PASS: standalone DMR Master display v2 installed. Backup: {backup}')
PY

php -l "$STATUS"
echo 'PASS: DMR Master card now preserves the last DMR lookup network outside DMR modes.'
