...
old_marker='// DVSwitch-Mode-Buttons: STFU Net card v5'
if s.count(old_marker) != 1: raise SystemExit('ERROR: expected exactly one STFU card v5')
a=s.find(old_marker); b=s.find('echo "</table>\\n";',a)
if b < 0: raise SystemExit('ERROR: STFU card end not found')
b += len('echo "</table>\\n";')
new='''// DVSwitch-Mode-Buttons: STFU Net card v6
$stfuStartTG = '';
$stfuInSection = false;
...
print('PASS: STFU card now reads StartTG with inline comments supported.')
PY
