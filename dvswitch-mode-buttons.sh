...
if [[ $alternate_net == BM ]]; then
  ...
  if [[ -z "$bm_password" || "$bm_password" == "passw0rd" ]]; then
    ...
    [[ "$bm_password" != "passw0rd" ]] || bm_password=""
  fi
else
  ...
  if [[ -z "$tgif_password" || "$tgif_password" == "passw0rd" ]]; then
    ...
    [[ "$tgif_password" != "passw0rd" ]] || tgif_password=""
  fi
fi
...
[[ $alternate_net == BM && $bm_ok != 1 ]] && echo "BM preset not enabled: required BM data was missing or the default password remains configured."
[[ $alternate_net == TGIF && $tgif_ok != 1 ]] && echo "TGIF preset not enabled: required TGIF data was missing or the default password remains configured."
